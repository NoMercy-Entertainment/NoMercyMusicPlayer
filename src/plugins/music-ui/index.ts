...
import type { MusicEventMap, MusicPlaylistItem } from '../../types';
...
export class MusicUiPlugin extends Plugin<NMMusicPlayer, MusicUiOptions, MusicUiEvents> {
    ...
    private buildArt(): ArtRefs {
        ...
        return { artWrap, artImg, artPlaceholder };
    }
    ...
    private buildTrackInfo(): TrackInfoRefs {
        ...
        return { infoWrap, titleEl, artistEl, albumEl };
    }
    ...
    private buildProgressRow(): ProgressRefs {
        ...
        return { progressRow, currentTimeEl, seekBar, seekBuffer, seekFill, seekThumb, durationTimeEl };
    }
    ...
    private makeBtn(action: string, iconKey: MusicIconKey, priority: number): HTMLButtonElement {
        ...
        if (priority > 0) btn.dataset['priority'] = String(priority);
    ...
    private currentSeekRatio(): number {
        if (this.cachedDuration <= 0) return 0;
    ...
    private updateBufferFill(): void {
        if (this.cachedDuration <= 0) return;
