...
import {AudioMotionAnalyzer, audioMotion} from "./audiomotion-analyzer";
...
export default class AudioNode<S extends Song> {
	...
	// Android TV
	protected isTv = window.matchMedia('(width: 960px) and (height: 540px)').matches;
	...
	private _initializeContext(): void {
		...
		if (this.isTv || localStorage.getItem('nmplayer-music-supports-audio-context') === 'false') return;

		if (!this.context) {
			try {
				this.motion = audioMotion(this._audioElement);
				this.motion.registerGradient('theme', {
					bgColor: 'transparent',
					dir: 'h',
					colorStops: [
						getComputedStyle(document.getElementById('audio-color') as HTMLDivElement).backgroundColor
					]
				});
